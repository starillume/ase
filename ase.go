...
func magicNumber(magic, number uint16, from ...string) {
...
func readChunk(reader io.Reader, buffer *bytes.Buffer, chunk []byte) error {
...
func temespacope(size int, buffer *bytes.Buffer) bool {
...
func negocioAntesDeLerELerTbmERetornarOObj[T any](size int, buffer *bytes.Buffer) T {
	var t T
	if temespacope(size, buffer) {
	...
	readChunk(Reader, buffer, ChunkPart)
	return negocioAntesDeLerELerTbmERetornarOObj[T](size, buffer)
}

func cuidardochunk(ch ChunkHeader, buffer *bytes.Buffer) []byte {
	size := ch.Size - ChunkHeaderSize
	if temespacope(int(size), buffer) {
	...
	readChunk(Reader, buffer, ChunkPart)
	return cuidardochunk(ch, buffer)
}
...
func main() {
	...
	magicNumber(0xA5E0, h.MagicNumber, "header")
	...
	for i := range h.Frames {
		...
		fh = negocioAntesDeLerELerTbmERetornarOObj[FrameHeader](FrameHeaderSize, buffer)
		magicNumber(0xF1FA, fh.MagicNumber, "frameheader", fmt.Sprint(i))
		...
		for range fh.ChunkNumber {
			...
			ch = negocioAntesDeLerELerTbmERetornarOObj[ChunkHeader](ChunkHeaderSize, buffer)

			fmt.Println("Chunk type: ", ch.Type)
			fmt.Println("Chunk size: ", ch.Size)

			fmt.Println("chunkdata to struct")
			bufchunk := cuidardochunk(ch, buffer)
